...
Реализует отправку запросов на сервер. В конструктор принимает 2 аргумента: ссылку на адрес сервера и и объект с заголовками запросов.
...
Класс отвечает за хранение данных одного товара, добавление товара в корзину. Конструктор класса принимает экземпляр брокера событий.
...
Класс отвечает за хранение выбранных товаров и удаление товаров из корзины. Конструктор класса принимает инстант брокера событий.
...
Класс отвечает за хранение данных пользователя, которые будут указаны им при оформлении заказа. Конструктор класса принимает инстант брокера событий.
...
Реализует модальное окно. Содержит методы open, close для открытия и закрытия окна. Устанавливает слушатели на клик по оверлею и кнопке-крестик для закрытия окна. Описывает логику вставки содержимого темплейт-элемента в верстку. 
...
Расширяет класс Modal. Реализует модальные окна с формами. Содержит методы для отображения ошибки при пустых полях ввода.
...
3 Класс ModalSuccess

Расширяет класс Modal. Реализует модальное окно с уведомлением об успешном оформлении заказа. Содержит методы для вывода списанной суммы, устанавливает слушатель на кнопку "за новыми покупками" для закрытия окна.

4 Класс ModalProductFull

Расширяет класс Modal. Реализует модальное окно карточки товара.
...
Расширяет класс Modal. Реализует модальное окно с корзиной. Отображает список выбранных товаров, выводит сумму за все товары.
...
Отвечает за отображение карточки товара. В конструктор передается template и экземпляр класса EventEmitter.
...
- render(): HTMLElement - возвращает заполненную карточку товарас установленными слушателями,
- геттер возвращает уникальный идентификатор товара.

7 Класс ProductContainer 

Отображает массив товаров на главной странице. 

Поля и методы класса:
- events: IEvents - экземпляр класса EventEmitter для инициации событий,
- render(): void - отрисовывает все товары в галерее.

### Слой коммуникации

Класс AppApi

Принимает в конструктор экземпляр класса Api и предоставляет методы для взаимодействия с бэкендом.
